...
def writeRec9():
    ...
    for t in range(nfiles):
        ...
        for k in range(NZ):
            ...
            CLDMRvals = eccodes.codes_get_values(int(gidRH[k]))
            CLDMRgrd[:, :, k] = np.reshape(CLDMRvals, (Nj, Ni), 'C')           
            RAINMRvals = eccodes.codes_get_values(int(gidRH[k]))
            RAINMRgrd[:, :, k] = np.reshape(RAINMRvals, (Nj, Ni), 'C')           
            ICEMRvals = eccodes.codes_get_values(int(gidRH[k]))
            ICEMRgrd[:, :, k]  = np.reshape(ICEMRvals, (Nj, Ni), 'C')       
            SNOWMRvals = eccodes.codes_get_values(int(gidRH[k]))
            SNOWMRgrd[:, :, k] = np.reshape(SNOWMRvals, (Nj, Ni), 'C')         
            GRPMRvals = eccodes.codes_get_values(int(gidRH[k]))
...
for i in range(nfiles):
    fileNames.append('{0}.{1}{2:03d}.{3}'.format(filePrefix, startDate, iFile,fileSuffix))
